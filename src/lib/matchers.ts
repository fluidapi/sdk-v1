...
import { FluidapiDefaultError } from "../models/errors/fluidapi-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | FluidapiDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | FluidapiDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new FluidapiDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new FluidapiDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
